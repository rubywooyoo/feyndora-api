...
def get_course_review(course_id):
    ...
    # 查詢課程評價資料（加入 good_points 和 improvement_points）
    cursor.execute("""
        SELECT accuracy_score, understanding_score, expression_score, interaction_score,
               teacher_comment, student1_feedback, student2_feedback, student3_feedback,
    ...
    if not review_data:
        # 預設的重點回顧陣列
        default_good_points = json.dumps([
            "概念解釋清晰準確",
            "舉例生動有趣",
            "與同學互動熱絡"
        ], ensure_ascii=False)
        
        default_improvement_points = json.dumps([
            "可以多分享實際應用場景",
            "建議控制節奏，不要說太快"
        ], ensure_ascii=False)
        ...
        cursor.execute("""
            ...
            (course_id, accuracy_score, understanding_score, expression_score, interaction_score,
             teacher_comment, student1_feedback, student2_feedback, student3_feedback,
             good_points, improvement_points)
            VALUES 
            (%s, 50, 50, 50, 50,
             ...
             %s, %s)
        """, (course_id, default_good_points, default_improvement_points))
